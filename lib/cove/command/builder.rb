  ...
  module Command
    class Builder
      ...
      def self.create_container(config)
        Docker::Container::Create.build(image: config.image, name: config.name, labels: config.labels, command: config.command, environment_files: config.environment_files, ports: config.ports, index: config.index, mounts: config.mounts)
      end
