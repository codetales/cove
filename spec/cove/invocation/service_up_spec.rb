  ...
  describe "#invoke" do
    context "with no existing containers" do
      it "should start a container" do
        registry, service, role = setup_environment(service_name: "test", role_name: "web", image: "app:latest", command: ["ping", "8.8.8.8"], ports: [{"type" => "port", "source" => 8080, "target" => 80}], volumes: [{"type" => "volume", "source" => "my-volume", "target" => "/data"}])
    ...
    def setup_environment(service_name: "test", role_name: "web", image: "app:latest", container_count: 1, command: [], ports: [], volumes: [])
      host = Cove::Host.new(name: "1.1.1.1")
      service = Cove::Service.new(name: service_name, image: image)
      role = Cove::Role.new(name: role_name, service: service, hosts: [host], container_count: container_count, command: command, ports: ports, volumes: volumes)
      registry = Cove::Registry.build(hosts: [host], services: [service], roles: [role])
