RSpec.describe Cove::Invocation::ServiceUp do
  describe "#invoke" do
    ...
    def setup_environment(service_name: "test", role_name: "web", image: "app:latest", container_count: 1)
      host = Cove::Host.new(name: "1.1.1.1")
      service = Cove::Service.new(name: service_name, image: image)
      role = Cove::Role.new(name: role_name, service: service, hosts: [host], container_count: container_count)
      registry = Cove::Registry.build(hosts: [host], services: [service], roles: [role])
