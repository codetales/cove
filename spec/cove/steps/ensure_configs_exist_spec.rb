RSpec.describe Cove::Steps::EnsureConfigsExist do
  describe "#call" do
    it "renders and uploads the configs to the host" do
      ...
      stubs { deployment_config.host_directories }.with { ["/var/cove/configs/STUBBED_VERSION", "/var/cove/configs/STUBBED_VERSION/postgres"] }
      stubs { deployment_config.files }.with { [file] }
      stubs { file.content }.with { "some content" }
      stubs { file.host_path }.with { "/var/cove/configs/STUBBED_VERSION/postgres/postgresql.conf" }

      mkdir_config_stub = stub_command(/#{Regexp.escape("mkdir -p /var/cove/configs/STUBBED_VERSION && chmod 700 /var/cove/configs/STUBBED_VERSION")}/)
      mkdir_entry_stub = stub_command(/#{Regexp.escape("mkdir -p /var/cove/configs/STUBBED_VERSION/postgres && chmod 700 /var/cove/configs/STUBBED_VERSION/postgres")}/)
      upload_stub = stub_upload("/var/cove/configs/STUBBED_VERSION/postgres/postgresql.conf").on("host1")
